...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  submitGuess: async () => {
    ...
    if (currentInput.length !== WORD_LENGTH) {
      shake(set);
      useToastStore.getState().show(
        currentInput.length === 0 ? 'Type a word' : 'Not enough letters'
      );
      return;
    }
    ...
    try {
      ...
      if (res.isSolved) {
        ...
        if (mode === 'daily') {
          ...
          useStatsStore.getState().recordPracticeResult(lang, true, puzzleId);
        }
        ...
        if (mode === 'daily') {
          ...
          useStatsStore.getState().recordPracticeResult(lang, false, puzzleId);
        }
      } else {
        set({
          guesses: nextGuesses,
...
/** Selector helpers / derived values exposed for tests. */
