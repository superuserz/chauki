...
async function unwrap<T>(res: Response): Promise<T> {
  ...
  if (!res.ok || body.error) {
    ...
    const apiError: ApiError = body.error ?? { code: 'INTERNAL', message: `HTTP ${res.status}` }
    throw new ApiClientError(res.status, apiError, retryAfter ? Number(retryAfter) : undefined)
  }
...
export const api = {
  ...
  todayPuzzle(lang: Language): Promise<PuzzleResponse> {
    return fetch(url(`/api/puzzles/today?lang=${encodeURIComponent(lang)}`), jsonInit('GET')).then(
      unwrap<PuzzleResponse>
    )
  },

  practicePuzzle(lang: Language, excludeRecent: string[] = []): Promise<PuzzleResponse> {
    return fetch(url('/api/puzzles/practice'), jsonInit('POST', { lang, excludeRecent })).then(
      unwrap<PuzzleResponse>
    )
  },
